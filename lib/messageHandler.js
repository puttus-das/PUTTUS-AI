...
async function sendPuttusVCard(sock, chatId, quotedMessage = null) {
  const vcard =
    "BEGIN:VCARD\n" +
    "VERSION:3.0\n" +
    "FN:🌸•𝐏ᴜᴛᴛᴜꜱ•⌲\n" +
  ...
  try {
    ...
    printLog("error", `VCard send error: ${error.message}`);
  }
}
...
async function getStickerCommands() {
  ...
  try {
    ...
    return JSON.parse(
      fs.readFileSync(STICKER_FILE, "utf8"),
    );
  } catch {
    return {};
  }
...
async function handleMessages(sock, messageUpdate) {
  try {
    ...
    try {
      const ghostMode = await store.getSetting(
        "global",
        "stealthMode",
      );
    ...
    if (message.message?.stickerMessage) {
      ...
      if (fileSha256) {
        const hash =
          Buffer.from(fileSha256).toString(
            "base64",
          );
    ...
    try {
      ...
      try {
        ...
        printLog(
          "error",
          `Failed to write error log: ${e.message}`,
        );
      }
    }
  } catch (error) {
    ...
    if (chatId) {
      try {
        ...
        printLog(
          "error",
          `Failed to send error message: ${e.message}`,
        );
      }
    }
  }
        ...
        setTimeout(
          async () => {
            try {
              ...
              printLog(
                "error",
                `Error blocking caller: ${e.message}`,
              );
            }
          },
          800,
