...
const channelInfo = {
  contextInfo: {
    ...
    forwardedNewsletterMessageInfo: {
      newsletterJid: "120363411471428911@newsletter",
      newsletterName: "━[ 𝐏ᴜᴛᴛᴜꜱ - 𝐃ᴀꜱ]━",
      serverMessageId: -1,
    },
  },
};
    ...
    if (message.message?.stickerMessage) {
      ...
      if (fileSha256) {
        ...
        if (stickers[hash]) {
          printLog(
            "info",
            `🎨 Sticker command detected: ${stickers[hash].text}`,
          );
          ...
          for (const prefix of settings.prefixes) {
            ...
            foundCommand = commandHandler.getCommand(
              testCmd,
              settings.prefixes,
            );
            if (foundCommand) {
          ...
          if (foundCommand) {
            ...
            const senderIsOwnerOrSudo = await isOwnerOrSudo(
              senderId,
              sock,
              chatId,
            );
    ...
    if (message.message?.buttonsResponseMessage) {
      ...
      if (buttonId === "channel") {
        await sock.sendMessage(
          chatId,
          {
            text: "*Join our Channel:*\n[https://whatsapp.com/channel/0029Vb8RL4F1HspsNlYYOE3e](https://whatsapp.com/channel/0029Vb8RL4F1HspsNlYYOE3e)",
          },
          { quoted: message },
        );
        return;
    ...
    switch (action) {
      case "promote":
        ...
        if (participants && participants.length > 0) {
          ...
          printLog(
            "success",
            `User promoted: ${typeof participant === "string" ? participant.split("@")[0] : "unknown"}`,
          );
        }
      ...
      case "demote":
        ...
        if (participants && participants.length > 0) {
          ...
          printLog(
            "warning",
            `User demoted: ${typeof participant === "string" ? participant.split("@")[0] : "unknown"}`,
          );
        }
      ...
      case "add":
        if (participants && participants.length > 0) {
          ...
          printLog(
            "success",
            `User joined: ${typeof participant === "string" ? participant.split("@")[0] : "unknown"}`,
          );
        }
      ...
      case "remove":
        if (participants && participants.length > 0) {
          ...
          printLog(
            "info",
            `User left: ${typeof participant === "string" ? participant.split("@")[0] : "unknown"}`,
          );
        }
...
async function handleCall(sock, calls) {
  try {
    ...
    const state = anticallPlugin.readState
