...
async function handleWelcome(sock, chatId, message, match) {
  ...
  if (lowerCommand === "on") {
    ...
    return sock.sendMessage(chatId, {
      text: "✅ Welcome messages *enabled* with simple message. Use *.welcome set [your message]* to customize.",
      quoted: message,
    });
  }
...
async function handleGoodbye(sock, chatId, message, match) {
  ...
  if (lower === "on") {
    ...
    return sock.sendMessage(chatId, {
      text: "✅ Goodbye messages *enabled* with simple message. Use *.goodbye set [your message]* to customize.",
      quoted: message,
    });
  }
  ...
  if (lower.startsWith("set ")) {
    ...
    if (!customMessage) {
      return sock.sendMessage(chatId, {
        text: "⚠️ Please provide a custom goodbye message. Example: *.goodbye set Goodbye!*",
        quoted: message,
      });
    }
