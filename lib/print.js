...
async function printMessage(message, sock) {
  try {
    ...
    const displayType =
      ...
      messageType.replace("Message", "").toUpperCase();
    ...
    console.log(
      chalk.gray(
        `[${message.key?.fromMe ? "ME" : "MSG"}] ${message.key?.remoteJid}`,
      ),
    );
  }
}
...
function printLog(type, message) {
  ...
  console.log(
    chalk.gray(`[${timestamp}]`) + " " + color(icon) + " " + color(message),
  );
}
