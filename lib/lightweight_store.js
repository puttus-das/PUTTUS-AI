...
if (MONGO_URL) {
  try {
      ...
      async cleanup() {
        try {
          const result = await Msg.deleteMany({
            ts: { $lt: Date.now() - TTL_MS },
          });
          if (result.deletedCount > 0) {
            console.log(
              `[MONGO] Cleaned up ${result.deletedCount} old messages`,
            );
          }
        } catch (e) {
    ...
    printLog(
      "store",
      "MongoDB enabled - Unlimited message storage with full data persistence",
    );
  } catch (e) {
...
if (backend === "memory" && POSTGRES_URL) {
  try {
    ...
    adapters.postgres = {
      ...
      async getAllCounts() {
        try {
          ...
          try {
            const res = await client.query(
              `SELECT chat_id, user_id, count FROM message_counts`,
            );
            const result = { isPublic: true, messageCount: {} };
            ...
            const metaRes = await client.query(
              `SELECT value FROM metadata WHERE key='isPublic'`,
            );
            if (metaRes.rows[0])
      ...
      async getChat(jid) {
        try {
          ...
          const client = await pool.connect();
          try {
            const res = await client.query(`SELECT * FROM chats WHERE jid=$1`, [
              jid,
            ]);
            return res.rows[0] || null;
          } finally {
      ...
      async getAllSettings(chatId) {
        try {
          ...
          try {
            const res = await client.query(
              `SELECT key, value FROM settings WHERE chat_id=$1`,
              [chatId],
            );
            const result = {};
            res.rows.forEach((row) => {
    ...
    printLog(
      "store",
      "PostgreSQL enabled - Unlimited message storage with full data persistence",
    );
  } catch (e) {
  ...
  try {
    ...
    adapters.mysql = {
      async getConn() {
        if (connectionFailed) {
          throw new Error(
            "MySQL connection permanently failed after multiple attempts",
          );
        }
        ...
        if (connectionAttempts >= MAX_RETRIES) {
          ...
          printLog(
            "error",
            "MySQL: Max connection attempts reached, disabling MySQL adapter",
          );
      ...
      async getChat(jid) {
        try {
          const conn = await this.getConn();
          const [rows] = await conn.execute(`SELECT * FROM chats WHERE jid=?`, [
            jid,
          ]);
          return rows[0] || null;
        } catch (e) {
      ...
      async cleanup() {
        try {
          ...
          if (result.affectedRows > 0) {
            console.log(
              `[MYSQL] Cleaned up ${result.affectedRows} old messages`,
            );
          }
        } catch (e) {
    ...
    printLog(
      "store",
      "MySQL enabled - Unlimited message storage with full data persistence",
    );
  } catch (e) {
...
if (backend === "memory" && SQLITE_URL) {
  try {
    ...
    sqlite
      .prepare(`CREATE INDEX IF NOT EXISTS idx_messages_jid ON messages(jid)`)
      .run();
    sqlite
    ...
    const saveStmt = sqlite.prepare(
      `INSERT OR REPLACE INTO messages VALUES (?,?,?,?)`,
    );
    const loadStmt = sqlite.prepare(
      `SELECT data FROM messages WHERE jid=? AND id=?`,
    );
    ...
    `);
    const getCountStmt = sqlite.prepare(
      `SELECT count FROM message_counts WHERE chat_id=? AND user_id=?`,
    );
    const getAllCountsStmt = sqlite.prepare(
      `SELECT chat_id, user_id, count FROM message_counts`,
    );
    const getMetaStmt = sqlite.prepare(
      `SELECT value FROM metadata WHERE key='isPublic'`,
    );
    const setMetaStmt = sqlite.prepare(
      `INSERT OR REPLACE INTO metadata(key, value) VALUES('isPublic', ?)`,
    );
    const getMetadataStmt = sqlite.prepare(
      `SELECT value FROM metadata WHERE key=?`,
    );
    ...
    const saveContactStmt = sqlite.prepare(
      `INSERT OR REPLACE INTO contacts(jid, name, notify, verified_name, ts) VALUES(?, ?, ?, ?, ?)`,
    );
    const getContactStmt = sqlite.prepare(`SELECT * FROM contacts WHERE jid=?`);
    const getAllContactsStmt = sqlite.prepare(
      `SELECT jid, name, notify FROM contacts`,
    );

    const saveChatStmt = sqlite.prepare(
      `INSERT OR REPLACE INTO chats(jid, name, conversation_timestamp, unread_count, ts) VALUES(?, ?, ?, ?, ?)`,
    );
    ...
    const saveSettingStmt = sqlite.prepare(
      `INSERT OR REPLACE INTO settings(chat_id, key, value, ts) VALUES(?, ?, ?, ?)`,
    );
    const getSettingStmt = sqlite.prepare(
      `SELECT value FROM settings WHERE chat_id=? AND key=?`,
    );
    const getAllSettingsStmt = sqlite.prepare(
      `SELECT key, value FROM settings WHERE chat_id=?`,
    );
    ...
    printLog(
      "store",
      `SQLite enabled - Max ${MESSAGE_LIMITS.sqlite} messages per chat with full data persistence`,
    );
  } catch (e) {
...
if (backend === "memory") {
  printLog(
    "store",
    `Using memory + JSON file storage - Max ${MESSAGE_LIMITS.memory} messages per chat`,
  );
}
...
const store = {
      ...
      for (const msg of messages) {
        ...
        if (backend === "memory") {
          ...
          try {
            ...
            console.error(
              `[STORE] Failed to save message ${msg.key.id}:`,
              e.message,
            );
          }
        }
          ...
          if (backend === "memory") {
            ...
            this.chats[chat.id] = {
              ...
              conversationTimestamp:
                chat.conversationTimestamp || existing.conversationTimestamp,
              unreadCount:
            ...
            try {
              ...
              const chatData = {
                ...
                name: chat.name || chat.subject || existing.name || "",
                conversationTimestamp:
                  chat.conversationTimestamp || existing.conversation_timestamp,
                unreadCount:
                  chat.unreadCount !== undefined
                    ? chat.unreadCount
  ...
  async incrementMessageCount(chatId, userId) {
    if (backend === "memory") {
      ...
      try {
        ...
        console.error(
          `[STORE] Failed to increment count for ${userId}:`,
          e.message,
        );
      }
    }
  ...
  getStats() {
    ...
    return {
      ...
      chats: totalChats,
      messageCounts:
        backend === "memory" ? totalMessageCounts : "stored in database",
      maxMessagesPerChat:
        messageLimit === Infinity ? "unlimited" : messageLimit,
      isPublic: this.isPublic,
...
console.log(
  `[STORE] Message limit per chat: ${messageLimit === Infinity ? "unlimited" : messageLimit}`,
);
