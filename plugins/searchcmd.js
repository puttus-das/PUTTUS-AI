  ...
  async handler(sock, message, args, context = {}) {
    ...
    if (!query) {
      return await sock.sendMessage(
        chatId,
        { text: "What are you looking for? Example: *.find status*" },
        { quoted: message },
      );
    }
    ...
    try {
      ...
      const results = allCommands.filter((commandObject) => {
        ...
        const descMatch = commandObject.description
          ?.toLowerCase()
          .includes(query);
        const aliasMatch = commandObject.aliases?.some((a) =>
      ...
      results.forEach((res, index) => {
        const status = CommandHandler.disabledCommands.has(
          res.command.toLowerCase(),
        )
          ? "🔸"
          : "🔹";
