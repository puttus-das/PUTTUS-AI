  ...
  async handler(sock, message, args) {
    ...
    if (mathGames[chatId]) {
      return sock.sendMessage(
        chatId,
        { text: "⚠️ Solve the current problem first!" },
        ...
        if (body == game.math.result) {
          ...
          if (game.attempts <= 0) {
            ...
            await sock.sendMessage(
              chat,
              {
                text: `❌ *Game Over!*\n\nThe correct answer was: *${game.math.result}*`,
              },
              { quoted: m },
            );
          } else {
            await sock.sendMessage(
              chat,
              {
                text: `❎ *Wrong answer!*\n\nYou have ${game.attempts} attempts left.`,
              },
              { quoted: m },
            );
          }
...
function randomInt(from, to) {
  ...
  return Math.floor(
    Math.random() * (Math.floor(to) - Math.ceil(from) + 1) + Math.ceil(from),
  );
}
