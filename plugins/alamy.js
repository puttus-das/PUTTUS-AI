  ...
  async handler(sock, message, args, context = {}) {
    ...
    try {
      ...
      for (const item of data.result) {
        if (item.video) {
          await sock.sendMessage(
            chatId,
            { video: { url: item.video }, caption: "🎬 *Alamy Video*" },
            { quoted: message },
          );
        }
        if (item.image) {
          await sock.sendMessage(
            chatId,
            { image: { url: item.image }, caption: "🖼️ *Alamy Image*" },
            { quoted: message },
          );
        }
      }
      ...
      if (error.code === "ECONNABORTED") {
        ...
        await sock.sendMessage(
          chatId,
          { text: "❌ Failed to download media from Alamy URL." },
          { quoted: message },
        );
      }
    }
