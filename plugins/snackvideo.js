  ...
  async handler(sock, message, args, context = {}) {
    ...
    if (!url) {
      return await sock.sendMessage(
        chatId,
        {
          text: "Please provide a SnackVideo URL.\nExample: .snack https://sck.io/p/...",
        },
        { quoted: message },
      );
    }
    ...
    try {
      ...
      if (!data?.status || !data.result?.length) {
        return await sock.sendMessage(
          chatId,
          { text: "❌ No media found for this SnackVideo URL." },
          { quoted: message },
        );
      }
      ...
      for (let mediaItem of data.result) {
        if (mediaItem.video) {
          await sock.sendMessage(
            chatId,
            { video: { url: mediaItem.video }, caption: "📹 SnackVideo Video" },
            { quoted: message },
          );
        }
        if (mediaItem.image) {
          await sock.sendMessage(
            chatId,
            { image: { url: mediaItem.image }, caption: "🖼 SnackVideo Image" },
            { quoted: message },
          );
        }
      }
      ...
      if (error.code === "ECONNABORTED") {
        ...
        await sock.sendMessage(
          chatId,
          { text: "❌ Failed to fetch SnackVideo media." },
          { quoted: message },
        );
      }
    }
