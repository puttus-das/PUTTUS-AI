  ...
  async handler(sock, message, args, context = {}) {
    ...
    try {
      if (action === "toggle") {
        ...
        return await sock.sendMessage(
          chatId,
          { text: `✅ Command *${targetCmd}* has been *${state}*.` },
          { quoted: message },
        );
      }
      ...
      if (action === "alias") {
        ...
        if (!targetCmd || !newAlias) {
          return await sock.sendMessage(
            chatId,
            { text: "❌ Usage: .manage alias [command] [new_alias]" },
            { quoted: message },
          );
        }
        ...
        if (!CommandHandler.commands.has(targetCmd)) {
          return await sock.sendMessage(
            chatId,
            { text: `❌ Source command *${targetCmd}* not found.` },
            { quoted: message },
          );
        }
        ...
        return await sock.sendMessage(
          chatId,
          { text: `✅ Added alias *${newAlias}* for command *${targetCmd}*.` },
          { quoted: message },
        );
      }
