  ...
  async handler(sock, message, args, context = {}) {
    ...
    try {
      const { data } = await axios.get(
        `https://discardapi.dpdns.org/api/stalk/pinterest`,
        {
      ...
      const profileImage =
        result.image?.large || result.image?.original || null;
      ...
      if (profileImage) {
        await sock.sendMessage(
          chatId,
          { image: { url: profileImage }, caption: caption },
          { quoted: message },
        );
      } else {
      ...
      await sock.sendMessage(
        chatId,
        { text: "❌ Failed to fetch Pinterest profile." },
        { quoted: message },
      );
    }
  },
