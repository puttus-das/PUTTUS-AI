...
module.exports = {
  ...
  category: "tools",
  description:
    "Get full SEO analysis of a website (split into multiple messages for WhatsApp)",
  usage: ".seo <url>",
  ...
  async handler(sock, message, args, context = {}) {
    ...
    if (!url) {
      return await sock.sendMessage(
        chatId,
        {
          text: "*Provide a website URL.*\nExample: .seo https://discardapi.dpdns.org",
        },
        { quoted: message },
      );
    }
    ...
    try {
      ...
      await sendSection(
        "Verification",
        r.verification.map((v) => `${v.name}: ${v.value} (${v.description})`),
      );

      await sendSection(
        "Metrics",
        r.metrics.map((m) => `${m.name}: ${m.score}`),
      );

      await sendSection(
        "Charset",
        r.charset?.map((c) => `${c.name}: ${c.charset} (${c.details})`),
      );
      ...
      if (r.cards?.length) {
        await sendSection(
          "Traffic Stats",
          r.cards.map((c) => `${c.title} (${c.subtitle}): ${c.value}`),
        );
      }
      ...
      if (r.speed_tips?.length) {
        await sendSection(
          "SEO Tips",
          r.speed_tips.map((t) => `${t.name}: ${t.details}`),
        );
      }
      ...
      if (r.broken_links?.length) {
        await sendSection(
          "Broken Links",
          r.broken_links.map((l) => `${l.text} (${l.url}) Status: ${l.status}`),
        );
      }
      ...
      if (r.domain_available?.length) {
        await sendSection(
          "Domain Availability",
          r.domain_available.map((d) => `${d.domain}: ${d.status}`),
        );
      }

      if (r.information_server?.header_text) {
        await sendSection("Server Info", [
          `Headers: ${r.information_server.header_text}`,
        ]);
      }
    } catch (error) {
      ...
      if (error.code === "ECONNABORTED") {
        await sock.sendMessage(
          chatId,
          { text: "❌ Request timed out. The API may be slow or unreachable." },
          { quoted: message },
        );
      } else {
        await sock.sendMessage(
          chatId,
          { text: "❌ Failed to fetch SEO information." },
          { quoted: message },
        );
      }
    }
