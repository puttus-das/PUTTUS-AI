  ...
  async handler(sock, message, args, context = {}) {
    ...
    try {
      ...
      if (
        ...
        !apiResponse.data?.data?.files ||
        apiResponse.data.data.files.length === 0
      ) {
        return await sock.sendMessage(
          chatId,
      ...
      if (error.code === "ETIMEDOUT" || error.code === "ECONNABORTED") {
        errorMsg +=
          "*Reason:* Timeout - File might be too large or connection is slow";
      } else if (
        error.code === "ECONNRESET" ||
        error.message?.includes("aborted")
      ) {
        errorMsg +=
          "*Reason:* Connection reset - The download was interrupted\nPlease try again.";
      } else if (error.response?.status === 429) {
        errorMsg +=
          "*Reason:* Rate limit exceeded\nPlease wait a minute and try again.";
      } else if (error.response?.status === 403) {
        errorMsg +=
          "*Reason:* Access forbidden - Link might be private or expired";
      } else if (error.response?.status === 404) {
        errorMsg +=
          "*Reason:* File not found - Link might be invalid or deleted";
      } else {
