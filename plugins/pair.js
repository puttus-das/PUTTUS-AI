...
module.exports = {
  ...
  category: "general",
  description: "Get session id for PUTTUS-AI",
  usage: ".pair 91896736XXXX",
  ...
  async handler(sock, message, args, context = {}) {
    ...
    try {
      const response = await axios.get(
        `https://puttus-pair-2a0c23da2b0b.herokuapp.com/pair?number=${number}`,
        {
          timeout: 60000,
        },
      );
      ...
      if (response.data && response.data.code) {
        ...
        if (
          pairingCode.includes("Unavailable") ||
          pairingCode.includes("Error")
        ) {
          throw new Error("Server is busy");
        }

        const successText =
          `✅ *PUTTUS-AI PAIRING CODE*\n\n` +
          `Code: *${pairingCode}*\n\n` +
          `*How to use:*\n` +
