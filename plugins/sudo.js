  ...
  async handler(sock, message, args, context = {}) {
    ...
    if (!isOwner) {
      await sock.sendMessage(
        chatId,
        {
          text: "❌ *Access Denied:* Only the Main Owner can manage Sudo privileges.",
        },
        { quoted: message },
      );
      return;
    ...
    if (!targetJid) {
      await sock.sendMessage(
        chatId,
        {
          text: "❌ Please mention a user, reply to a message, or provide a number.",
        },
        { quoted: message },
      );
      return;
    ...
    if (targetJid.includes("@lid") && isGroup) {
      try {
        ...
        const found = metadata.participants.find(
          (p) => p.lid === targetJid || p.id === targetJid,
        );
    ...
    if (sub === "del" || sub === "remove") {
      ...
      if (displayId === ownerNumberClean) {
        await sock.sendMessage(
          chatId,
          { text: "❌ *Action Denied:* Cannot remove the Main Owner." },
          { quoted: message },
        );
        return;
      }
